...
- **Linux is CPU-only and needs the right mlx backend.** The `mlx` wheel ships only the Python bindings; its bundled `libmlx.so` is a Metal stub that fails to import on Linux (`libmlx.so: cannot open shared object file`), and mlx-lm depends on `mlx` only under Darwin. `pyproject.toml` therefore pulls `mlx[cpu]` on Linux (the `cpu` extra adds the ABI-matched `mlx-cpu` backend) and plain `mlx` elsewhere (Darwin auto-adds `mlx-metal`). Bare `mlx-cpu` is NOT enough - it has the backend lib but no `mlx.core` bindings. On Linux the four Metal-gated parity tests skip (`mx.metal.is_available()` is False); CPU parity gates still run and must stay green.
...
- **`sentrux check .` and `sentrux gate .` enforce different things.**
  ...
  sentrux itself is fleet-local tooling: it is not installable from this repo and not required to contribute - `pytest` remains the contributor check, and the committed `.sentrux/` files (`rules.toml`, `baseline.json`) are tuned config, not a dependency.
- **Measured headroom (sentrux 0.5.7):** 0 cycles, 0 god files, and `mlx_dllm/diffusion.py:denoise` is the ceiling on both function metrics at cc=13 and 84 lines.
  ...
  Unlike `max_cc`, `max_fn_lines` is not overridden: it binds at exactly its configured value, but only under `sentrux check` - `sentrux gate` has no function-length metric at all (probe on 0.5.7: one 402-line function reported `exceed max length of 100 lines`, then `of 50 lines`, then `of 300 lines` as the configured value changed, and that same function at cc=1 passes gate clean).
